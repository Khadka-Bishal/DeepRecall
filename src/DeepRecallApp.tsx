...
interface DeepRecallAppProps {
  onHome: () => void;
}

const DeepRecallApp = ({ onHome }: DeepRecallAppProps) => {
  ...
  const {
    ...
    onError: (msg) => addToast({ type: 'error', title: 'Error', message: msg })
  });
  ...
  return (
    <div className="flex h-screen bg-[#09090b] text-zinc-300 font-sans overflow-hidden selection:bg-zinc-800 selection:text-zinc-100">
      ...
      <div className="flex-1 flex flex-col relative bg-[#09090b]">
        {/* Header */}
        <div className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 bg-[#09090b] z-10">
          <div 
            className="flex items-center gap-3 text-zinc-100 mr-4 cursor-pointer hover:opacity-80 transition-opacity"
            onClick={onHome}
          >
            <DeepRecallLogo />
            <span className="font-grotesk font-semibold text-lg tracking-tight">DeepRecall</span>
          </div>
          <div className="flex items-center gap-2">
            <button
               onClick={() => setShowMobileInspector(!showMobileInspector)}
               className="lg:hidden p-2 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 rounded-full"
            >
              <Layers size={18} />
            </button>
            <button
              tabIndex={-1}
              onClick={resetState}
              className="flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 px-4 py-2 rounded-full border border-zinc-800 text-xs font-medium group focus:outline-none focus:ring-0 transition-all hover:border-zinc-700"
            >
          ...
          <div className="max-w-4xl mx-auto py-8 px-6">
            {messages.length === 0 ? (
              <div className="mt-20 flex flex-col items-center justify-center text-center">
                ...
                <h2 className="text-2xl font-bold text-zinc-100 mb-3 font-grotesk">Start RAG Pipeline</h2>
                ...
                {!uploadedFile && (
                  <button
                    ...
                    className="group flex items-center gap-3 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 px-6 py-3 rounded-full focus:outline-none focus:ring-0 transition-all"
                  >
                  ...
                  <div key={msg.id} className="group">
                    <div className="flex items-baseline justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {msg.role === 'assistant' ? 'DeepRecall' : 'You'}
                        </span>
      ...
      {/* Right Panel (Inspector) - Responsive: Hidden on mobile unless toggled, fixed width on desktop */}
      <div 
        className={`${
          activeTab === 'pipeline' || activeTab === 'retrieval' ? 'flex' : 'hidden'
        } lg:flex lg:w-[400px] w-full border-l border-zinc-800 bg-[#09090b] flex-col shadow-xl z-20 absolute lg:relative inset-0 lg:inset-auto h-full lg:h-auto`}
        style={window.innerWidth < 1024 && !showMobileInspector ? { display: 'none' } : {}}
      >
        <div className="flex border-b border-zinc-800 relative">
           {/* Mobile Close Button */}
           <button 
             onClick={() => setShowMobileInspector(false)}
             className="lg:hidden absolute left-2 top-3 text-zinc-500 hover:text-zinc-300 z-50 p-1"
           >
            ...
            <div className="p-5">
              ...
              {pipelineStatus === PIPELINE_STEPS.IDLE ? (
                ...
                <div className="pl-2 pt-2">
                  ...
                  {/* AWS Pipeline Steps (Visual Only) */}
                  <PipelineStepIndicator
                    label="AWS Cloud Handoff"
                    icon={Workflow}
                    status={mapStepStatus(pipelineStatus, 'securing')}
                    details={mapStepStatus(pipelineStatus, 'securing') === 'active' ? 'Securing & Orchestrating...' : undefined}
                  />
              ...
              {(() => {
                ...
                <>
                  {expandedQueries.length > 0 &&
                    messages[messages.length - 1]?.retrievedChunks &&
                    messages[messages.length - 1]!.retrievedChunks!.length > 0 && (
                  ...
                  <div className="bg-zinc-900/50 px-2 py-1.5 border-b border-zinc-800 flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        setFullViewStep({
                          type: 'chunks',
                          items: messages[messages.length - 1]!.retrievedChunks!,
                        })
                      }
                      className="flex items-center gap-1 text-[9px] font-mono text-zinc-500 hover:text-zinc-300"
                    >
                      <span>{messages[messages.length - 1]?.retrievedChunks?.length} items</span>
                      <Maximize2 size={10} />
                    </button>
                  </div>
                  {messages[messages.length - 1]?.retrievedChunks?.map((chunk) => (
